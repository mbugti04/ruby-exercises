...
def find_beautiful_languages(languages)
  ...
  # languages.find{|lang| lang[:is_beautiful?] == true }
  # p languages.select{|lang, data| p lang; p data; data[:is_beautiful?] == true}
  languages.select{|lang, data| data[:is_beautiful?] == true}
end
...
def find_language_facts(languages, language_name, fact_index = 0)
  ...
  # dig is better because if one level is nil, then it does not raise an error
